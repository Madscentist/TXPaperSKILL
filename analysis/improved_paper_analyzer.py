...
class ImprovedPaperAnalyzer:
    ...
    def identify_sections_by_headings(self, text: str) -> Dict[str, str]:
        """通过明确的section标题识别section"""
        sections = {}
        
        # 查找所有可能的section标题
        # 匹配模式：罗马数字 + 标题，或大写字母 + 标题
        heading_patterns = [
            r'(?:^|\n)\s*(?:I|II|III|IV|V|VI|VII|VIII|IX|X)\.\s*([A-Z][A-Z\s]+)',  # I. INTRODUCTION
            r'(?:^|\n)\s*(?:I|II|III|IV|V|VI|VII|VIII|IX|X)\.\s*([A-Z][a-z\s]+)',  # I. Introduction
            r'(?:^|\n)\s*([A-Z][A-Z\s]{2,})\s*(?:\n|$)',  # INTRODUCTION (全大写)
    ...
    def analyze_paper(self, filename: str) -> Dict:
        ...
        # 识别section
        sections = self.identify_sections_by_headings(text)
...
if __name__ == "__main__":
    analyzer = ImprovedPaperAnalyzer("/mnt/g/project/PaperSkill/extracted_texts")
    analyzer.analyze_all_papers()
    analyzer.save_analysis("/mnt/g/project/PaperSkill/analysis/improved_papers_analysis.json")