...
class SimplePaperAnalyzer:
    ...
    def find_section_titles(self, text: str) -> List[Tuple[str, int, str]]:
        """查找所有section标题"""
        titles = []
        ...
        if abstract_match:
            titles.append(('abstract', abstract_match.start(), abstract_match.group(0)))
        ...
        for match in re.finditer(roman_pattern, text):
            ...
            elif 'RELATED' in section_text or 'LITERATURE' in section_text:
                section_name = 'related_work'
            elif 'METHOD' in section_text or 'PROPOSED' in section_text:
                section_name = 'methods'
            elif 'EXPERIMENT' in section_text or 'RESULT' in section_text:
                section_name = 'results'
            elif 'DISCUSSION' in section_text:
                section_name = 'discussion'
            elif 'CONCLUSION' in section_text:
                section_name = 'conclusion'
            else:
                continue
            
            titles.append((section_name, start_pos, match.group(0)))
    ...
    def extract_sections(self, text: str, titles: List[Tuple[str, int, str]]) -> Dict[str, str]:
        """提取每个section的内容"""
        sections = {}
        
        for i, (section_name, start_pos, title_text) in enumerate(titles):
            if i < len(titles) - 1:
                next_start = titles[i + 1][1]
    ...
    def analyze_paper(self, filename: str) -> Dict:
        ...
        # 查找section标题
        titles = self.find_section_titles(text)
        
        # 提取section内容
        sections = self.extract_sections(text, titles)
...
if __name__ == "__main__":
    analyzer = SimplePaperAnalyzer("/mnt/g/project/PaperSkill/extracted_texts")
    analyzer.analyze_all_papers()
    analyzer.save_analysis("/mnt/g/project/PaperSkill/analysis/simple_papers_analysis.json")