...
class StyleAnalyzer:
    ...
    def analyze_syntactic_style(self, text: str) -> Dict:
        ...
        nominalization_count = sum(1 for word in word_tokenize(text) 
        ...
        for ending in sentence_endings:
            if ending.endswith('.'):
                ending_patterns['period'] += 1
            elif ending.endswith(')'):
                ending_patterns['citation'] += 1
            elif ending.endswith(':'):
                ending_patterns['colon'] += 1
        ...
        return {
            ...
            'nominalization_ratio': nominalization_count / len(word_tokenize(text)) if text else 0,
            'sentence_endings': dict(ending_patterns)
        }
    ...
    def analyze_rhetorical_structure(self, paper_analysis: Dict) -> Dict:
        ...
        for section_name, section_data in paper_analysis['sections'].items():
            ...
            for para in section_data['paragraphs']:
                function_counts[para['function']] += 1
...
if __name__ == "__main__":
    analyzer = StyleAnalyzer("/mnt/g/project/PaperSkill/analysis/direct_papers_analysis.json")
    style_analyses = analyzer.analyze_all_papers()
    analyzer.save_style_analysis("/mnt/g/project/PaperSkill/analysis/style_analyses.json", style_analyses)